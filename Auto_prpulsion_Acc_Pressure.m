...
plot(Acc_St_RS_x_r * 200, 'k')
plot(Acc_St_RS_y_r * 200, 'y')
...
ls_HsR = length(HsR_pressure);
for o = 1:ls_HsR
   if(HsR_pressure(o) < 0)
       HsR_pressure(o) = [];
       ToR_pressure(o) = [];
       if(HsR_pressure(o+1) > 0)
           break;
       end
   end
end
...
for j = 1:length(Pro_Pressure)
    plot(Pro_Pressure(j)+temp_coeff,pressure_resample_whole(Pro_Pressure(j)),'*k','MarkerSize',20)
end
...
for j = length(res_St):-1:1+length(res_St)-length(Pro_Pressure)
    result(j).Pro_R_Pressure = Pro_Pressure(1,j-(length(res_St)-length(Pro_Pressure)))+temp_coeff;
end

prompt = 'Which points are not correct: ';
        delet_points = input(prompt)
        
        for l = length(delet_points): -1 : 1
            result(delet_points(l)) = [];
        end
...
switch answer
    ...
    case 'Yes'
        ...
        SavingDir = 'Z:\Projects BCM\H-38994 BLANKET BCM\Studies\Propulsion\Results\Propulsion_Points_Acc_Pressure';
