...
%                    pressure_resample_HeelTest = resample(pressure_HeelTest(:,5),2,1);
...
%            if (str2double(file(6:7)) == PatientNameDataDble && str2double(file2(6:7)) == PatientNameDataDble)
               ...
               %%
                   ...
                   if (points > res_St(3).HsR)
                      h(1)=subplot(2,1,1); hold on; plot(pressure_resample_whole(round(points)-res_St(3).HsR:end),'g');
...
%                         h(1)=subplot(2,1,1); hold on; plot(A5(round(points)-res_St(3).HsR:end) * 10,'black');
...
%                        h(1)=subplot(2,1,1); hold on; plot(DG* 50,'k');
                     ...
                     h(1)=subplot(2,1,1); hold on; plot(A5(round(points)-res_St(3).HsR:end)* 10,'c');
                     PressureNew = A5(round(points)-res_St(3).HsR:end);
                        for u = 2:length(res_St)-1
                            ...
                            h(1)=subplot(2,1,1); hold on; plot(res_St(u).HsR + pointpressureMax + 40,Gyro_St_RS_r(res_St(u).HsR + pointpressureMax + 40),'ok','markersize',12);
                                              
                        end
                   else
                      h(1)=subplot(2,1,1); hold on; plot(res_St(3).HsR - round(points):length(pressure_resample_whole)+res_St(3).HsR - round(points)-1,pressure_resample_whole,'g');
                    %   h(1)=subplot(2,1,1);  hold on; plot(res_St(3).HsR - round(points):length(pressure_resample_Heel)+res_St(3).HsR - round(points)-1,pressure_resample_Heel,'black');
%                        h(1)=subplot(2,1,1); hold on; plot(IntegGyro_St_RS_r,'k');
...
%                        plot(res_St(3).HsR - round(points):length(dpressureHeeldx)+res_St(3).HsR - round(points)-1,a5,'black');
                        ...
                        for u = 2:length(res_St)-1
                            pointpressureMax = find(PressureNew(res_St(u).HsR + 40:res_St(u).ToR) == max(PressureNew(res_St(u).HsR + 40:res_St(u).ToR)))
                            h(1)=subplot(2,1,1); hold on; plot(res_St(u).HsR+pointpressureMax + 40,Gyro_St_RS_r(res_St(u).HsR +40 +pointpressureMax),'ok','markersize',12);
...
% peak based on the swing movement in the other side
...
%                         PointforSwingadjR(l-1,2) = find(Gyro_St_RS_r(res_St(l).HsR:res_St(l).ToR) == PointforSwingadjR(l-1,1)) + res_St(l).HsR ;
                        ...
                        [peak,locs,widths,proms] = findpeaks(diff(Gyro_St_RS_r(PointforSwingadjR(l-1,1):res_St(l).ToR-5)));
                            biggestPeak = find(widths == max(widths))
                            if(size(peak,1) ~= 0 )
                                
                                PointofPeakGyr = find(diff(Gyro_St_RS_r(PointforSwingadjR(l-1,1):res_St(l).ToR-5)) == peak(biggestPeak));
...
%                    x = input('number of the peaks: ');
                   ...
                   for u = 1:x
                       pointToR(u,1) = res_St(u+1).ToR;
                   end
                   PointRight = [PointforPlotR pointofInterest pointToR]
